...
| **Drive Validator** | Non-destructive checks against a removable USB target's free space (Quick Safe Check, Sampled Capacity Check, Full Free-Space Validation) to flag suspicious capacity or failing media before building a toolkit. Writes only into `.forgerems-drive-validator` on the chosen USB; never formats, never deletes user files, and never runs against the Windows OS drive, system / boot / EFI / VTOYEFI partitions, or internal fixed disks by default. Results are advisory evidence for a technician, not a guarantee that the drive is genuine. Destructive full-media mode is not available in this build. |
