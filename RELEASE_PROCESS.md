...
2. Confirm large generated payloads are not staged: `*.iso`, `*.img`, `*.zip`, `*.exe`, `dist/`, and `release/` outputs stay ignored.
...
1. Confirm `release/<version>/app/ForgerEMS.exe` exists.
2. Confirm `release/<version>/backend/` contains the backend scripts, manifests, checksums, signature, and bundled metadata.
3. Confirm `release/<version>/manifests/` contains the public manifest files.
4. Review `release/<version>/CHECKSUMS.sha256`.
...
4. Attach the installer and checksum file from `release/<version>/`.
5. Mark Beta releases clearly in the release title, for example `ForgerEMS 1.0.0 Beta 1`.
