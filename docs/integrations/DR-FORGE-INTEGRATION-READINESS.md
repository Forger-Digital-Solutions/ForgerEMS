...
- `tests/ForgerEMS.Wpf.Tests/DrForgeCliBridgeTests.cs` pins conservative `forger-sensor-driver-preflight/1.1` parsing, current sanitized Dr. Forge `forge-sensor-core/1.0` / `forge-hardware-intake-report/1.0` fixtures, archive metadata fallback, non-fatal older-CLI behavior, null-as-unavailable report rendering, and no-network/no-elevation bridge behavior.
