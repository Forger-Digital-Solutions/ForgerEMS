# ForgerEMS v1.2.4-preview.4 Release Notes

ForgerEMS `v1.2.4-preview.4` is a preview/pre-release build for Windows technicians.
...
- Dr. Forge compatibility fixtures now track Dr. Forge source commit `433001c` and its `contracts/reports/` source-of-truth package.
...
- The Dr. Forge Intake card now shows richer local readiness: CLI version/commit when available, safe driver-status schema `forger-sensor-driver-preflight/1.1` summary, last successful scan/report time, recent app-managed report/archive history, Refresh Status, Open Report Folder, and Copy Status Summary. No driver artifacts are bundled or activated.
- Local Dr. Forge report preview now shows parsed read-only sections for known JSON report schemas, keeps a capped Raw Preview available, treats unknown schemas as raw/metadata fallback, and leaves ZIP/archive reports metadata-only with no extraction.
...
- `ForgerEMS-v1.2.4-preview.4.zip` portable app ZIP
- `CHECKSUMS.sha256`
