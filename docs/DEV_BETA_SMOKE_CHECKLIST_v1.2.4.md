...
- [ ] In the **Dr. Forge Intake** card, confirm **Local Dr. Forge report preview** is read-only/local-only copy, the recent report selector appears, **Parsed Sections** and **Raw Preview** tabs are present, parsed known-schema sections show report-derived values only, archive/ZIP selections say metadata-only, **Copy Report Summary** and **Open Containing Folder** are present, and no upload, driver install, elevation, fan/voltage/OC, or firmware-control action appears.
