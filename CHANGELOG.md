...
- **Dr. Forge contract source tracking:** Compatibility fixtures now record Dr. Forge source commit `433001c` and include copied source-owned contract metadata, driver-preflight, archive metadata, and unknown/future schema fallback fixtures.
...
- **Version bump:** app/project metadata, installer defaults, release tooling, validation defaults, docs, release notes, smoke checklist, and version-pinned tests moved from `1.2.4-preview.1` to `1.2.4-preview.4`. Windows assembly/file version remains `1.2.4.0`.
...
- **Local report preview:** Toolkit Manager can select recent app-managed Dr. Forge reports/archives/snapshots and show a bounded, read-only local preview: JSON summaries parse conservative known fields, Markdown is shown as capped plain text, ZIP archives stay metadata-only, and no upload or driver/elevation action is performed.
...
- **Richer Dr. Forge status/history display:** Toolkit Manager now separates CLI version/commit, driver-status safety summary, last successful scan/report time, and recent app-managed Dr. Forge report/archive history from the report body, with Refresh Status and Copy Status Summary actions. Report history stays bounded to the local Runtime Dr. Forge reports folder.
